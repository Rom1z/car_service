...
Python + PyQt6 + MySQL
С генерацией PDF отчётов и заказ-нарядов
"""
    ...
    def generate_order_pdf(order_id, filename):
        ...
        try:
            ...
            c.drawString(x_left, y,
                         f"Дата создания: {o['created_at'].strftime('%d.%m.%Y %H:%M') if o['created_at'] else '-'}")
            y -= 18
            ...
            c.setFont("Helvetica", 10)
            for p in parts:
                c.drawString(x_left, y, p['part_name'][:30])
                c.drawString(x_left + 200, y, p['article'] or "-")
...
# ============================================
# МОДУЛЬ "ЗАКАЗ-НАРЯДЫ" (ИСПРАВЛЕННЫЙ)
# ============================================
class OrderWidget(QWidget):
    ...
    def load_data(self):
        try:
            ...
            status_names = {'new': 'Новый', 'in_progress': 'В работе', 'completed': 'Выполнен', 'closed': 'Закрыт',
                            'cancelled': 'Отменён'}
            ...
            for i, row in enumerate(rows):
                ...
                self.table.setItem(i, 4, QTableWidgetItem(f"{row['total_labor']:.2f}"))
                self.table.setItem(i, 5, QTableWidgetItem(f"{row['total_parts']:.2f}"))
                self.table.setItem(i, 6, QTableWidgetItem(f"{row['final_total']:.2f}"))
        ...
        if ok:
            ...
            try:
                if new_status == 'closed':
                    ...
                    Database.execute_query(
                        "UPDATE repair_orders SET status=%s, completed_at=%s WHERE id=%s",
                        (new_status, datetime.now() if new_status in ('completed', 'closed') else None, order_id),
                        fetch=False
                    )
                self.load_data()
            except Error as e:
    ...
    def _close_order(self, order_id):
        ...
        try:
            ...
            total = (float(labor) + float(parts)) * (1 - float(discount) / 100)
    ...
    def print_order(self):
        ...
        if filename:
            if PDFGenerator.generate_order_pdf(order_id, filename):
                ...
                os.startfile(filename)  # Открыть PDF
            else:
...
class OrderDialog(QDialog):
    """Диалог заказ-наряда с РАБОЧИМ добавлением услуг и запчастей."""
    ...
    def __init__(self, user, order_id=None, parent=None):
        ...
        self.services_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Fixed)
        ...
        self.parts_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Fixed)
    ...
    def _add_service_row(self, name="Новая услуга", hours="1.0", rate="1000"):
        ...
        self.services_table.cellChanged.disconnect(self._update_totals)  # Отключаем сигнал
        ...
        self.services_table.cellChanged.connect(self._update_totals)  # Включаем обратно
        self._update_totals()

    def _add_part_dialog(self):
        """Диалог выбора запчасти из каталога."""
    ...
    def _update_totals(self):
        ...
        try:
            ...
            # Общий итог со скидкой
            discount = self.discount_spin.value()
            subtotal = labor_total + parts_total
            grand_total = subtotal * (1 - discount / 100)
    ...
    def load_order_data(self):
        try:
            ...
            self.discount_spin.setValue(float(d['discount_percent']) if d['discount_percent'] else 0)
    ...
    def save_order(self):
        ...
        try:
            if self.order_id:
                ...
                Database.execute_query(
                    "UPDATE repair_orders SET client_id=%s, vehicle_id=%s, mechanic_id=%s, description=%s, discount_percent=%s WHERE id=%s",
                    (client_id, vehicle_id, mechanic_id, desc, discount, self.order_id), fetch=False
                )
                order_id = self.order_id
            else:
                cursor = Database.get_connection().cursor()
                cursor.execute(
                    "INSERT INTO repair_orders (client_id, vehicle_id, mechanic_id, description, discount_percent, status) VALUES (%s,%s,%s,%s,%s,'new')",
...
class InventoryWidget(QWidget):
    ...
    def delete_part(self):
        ...
        if QMessageBox.question(self, "Удаление", "Удалить запчасть?") == QMessageBox.StandardButton.Yes:
            try:
                Database.execute_query("DELETE FROM parts_catalog WHERE id=%s", (int(self.table.item(row, 0).text()),),
                                       fetch=False)
                self.load_data()
            except Error as e:
                QMessageBox.critical(self, "Ошибка", str(e))
...
# ============================================
# МОДУЛЬ "ОТЧЁТЫ" (с экспортом в PDF)
# ============================================
